#################################################################
## Estimate reliability of t = 4 based on right censored data  ##
...
# There are 60 right censored data in total.
# 5 data points are censored at t = 40; 
# 55 data points are censored at t = 4; 

# Censoring time
# CenLimit <- c( rep(4,55), rep(40,5) )

CenLimit <- c( rep(40,20) )
...
BayesianData <- list(ttf = ttf,
                     CenLimit = CenLimit, Censor = Censor,  n=n
)
...
ModelObject <- jags.model(file = "Weibull_RightCensored.JAGS", 
                          data=BayesianData, inits=list(list(.RNG.name="base::Mersenne-Twister", .RNG.seed=1349, shape=1, scale=10, ttf = ttfInit1),
                                                        list(.RNG.name="base::Mersenne-Twister", .RNG.seed=6247, shape=1.5, scale=40, ttf = ttfInit2)), 
